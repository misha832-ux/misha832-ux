...
// Run locally: GITHUB_TOKEN=xxx node .github/scripts/gen-telemetry.mjs

import { writeFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";
import { fileURLToPath } from "node:url";

const USER = "misha832-ux";

// Project cards, in display order. `star` = "owner/repo" whose live star count
// is shown as "<lang> · ★ N". If `star` is null, or the repo lookup fails
// (e.g. not public yet), `meta` is used verbatim instead — nothing crashes.
const PROJECTS = [
  { key: "verse4u", title: "verse4u", lang: "typescript", star: "misha832-ux/verse4u",
    href: "https://github.com/misha832-ux/verse4u",
    d1: "quranic verses browsable by how you feel",
    d2: "next.js · drizzle orm · neon · better auth" },
  { key: "nobojatra", title: "nobojatra", lang: "typescript", star: "Sakif16/nobojatra",
    href: "https://github.com/Sakif16/nobojatra",
    d1: "dhaka transportation aggregator",
    d2: "next.js · leaflet · openrouteservice" },
];
...
async function collect() {
  ...
  // star counts for cards that show a star — never fatal if the repo lookup fails
  const stars = {};
  for (const p of PROJECTS) {
    ...
    try {
      ...
      stars[p.key] = null; // repo private/renamed/not found yet — card falls back to p.meta
    }
  }
...
// ── templates ───────────────────────────────────────────────────
// ── black / green palette (matches the hand-built panels) ───────
const STYLE = `<style>
...
for (const theme of ["dark", "light"]) {
  ...
  for (const p of PROJECTS) {
    const meta = p.star && data.stars[p.key] != null ? `${p.lang} · ★ ${data.stars[p.key]}` : p.meta ?? `${p.lang}`;
